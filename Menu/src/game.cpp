...
namespace Wine
{
  GameVariables::GameVariables() : m_GameState(Signature("48 85 C9 74 4B 83 3D", "GameState").Scan().Add(7).Rip().As<decltype(m_GameState)>()),
  ...
  GameFunctions::GameFunctions() : m_IsDlcPresent(Signature("84 C0 74 0D 8D 53 0C", "IsDlcPresent").Scan().Sub(63).As<decltype(m_IsDlcPresent)>()),
                                   ...
                                   m_SetCursorPos(Module("user32.dll").GetExport("SetCursorPos").As<decltype(m_SetCursorPos)>())
  {
  }
}
